...
// @desc    Get verification results (dashboard stat)
...
exports.getVerificationResults = async (req, res) => {
  try {
    ...
    const pending = participants.filter(p => p.Verification_Status === 'Pending').length;
...
// @desc    Get all attendees (dashboard stat)
...
// @desc    Get recent activities (dashboard feed)
...
exports.getRecentActivities = async (req, res) => {
  try {
    ...
    // Mock recent activities based on participants
    const activities = participants.slice(0, 10).map((p, index) => ({
      id: index + 1,
      action: 'Ticket Booked',
      user: p.user_name || 'Unknown',
      timestamp: p.Purchase_Date || new Date().toISOString(),
...
// @desc    Get event list (for filtering)
...
exports.getEventList = async (req, res) => {
  try {
    ...
    participants.forEach(p => {
      if (p.Event_ID && !eventMap[p.Event_ID]) {
        eventMap[p.Event_ID] = {
          Event_ID: p.Event_ID,
